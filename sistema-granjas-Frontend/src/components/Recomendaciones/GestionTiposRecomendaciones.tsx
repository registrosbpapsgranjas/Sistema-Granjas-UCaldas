...
import tipoRecomendacionService from '../../services/tipoRecomendacionesService';
...
const GestionTiposRecomendaciones: React.FC = () => {
    ...
    const cargar = async () => {
        setLoading(true);
        try {
            const data = await tipoRecomendacionService.obtenerTipos();
            setTipos(Array.isArray(data) ? data : []);
        } catch {
            toast.error('Error al cargar tipos de recomendación');
        } finally {
            setLoading(false);
        }
    };
    ...
    const guardar = async () => {
        ...
        try {
            ...
            if (editandoId) {
                await tipoRecomendacionService.actualizarTipo(editandoId, payload);
                toast.success('Tipo de recomendación actualizado');
            } else {
                await tipoRecomendacionService.crearTipo(payload);
                toast.success('Tipo de recomendación creado');
            }
            await cargar();
            cancelar();
        } catch (e: any) {
    ...
    const eliminar = async (id: number, nombre: string) => {
        if (!confirm(`¿Eliminar el tipo de recomendación "${nombre}"?`)) return;
        try {
            await tipoRecomendacionService.eliminarTipo(id);
            setTipos(prev => prev.filter(t => t.id !== id));
            toast.success('Tipo de recomendación eliminado');
        } catch (e: any) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-lg font-bold text-gray-800">Tipos de Recomendación</h2>
                    <p className="text-sm text-gray-500">Categorías para clasificar las recomendaciones emitidas en el sistema.</p>
                </div>
                <button onClick={abrirCrear}
                    className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium">
            ...
            {showForm && (
                <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
                    <h3 className="font-semibold text-orange-800 mb-4 text-sm">
                        {editandoId ? 'Editar tipo de recomendación' : 'Nuevo tipo de recomendación'}
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value={form.nombre}
                                ...
                                placeholder="Ej: Control fitosanitario, Fertilización, Riego..." />
                        </div>
                        <div>
            ...
            {loading ? (
                ...
                <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
                    <i className="fas fa-bookmark text-3xl text-gray-300 block mb-3"></i>
                    <p className="text-gray-500 font-medium">No hay tipos de recomendación registrados</p>
                ...
                <div className="bg-white rounded-xl shadow overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {tipos.map(tipo => (
                                <tr key={tipo.id} className="hover:bg-orange-50 transition-colors">
                                    <td className="px-5 py-3 text-sm font-mono text-gray-400">#{tipo.id}</td>
                                    <td className="px-5 py-3">
                                        <span className="inline-flex items-center gap-2 text-sm font-medium text-gray-900">
                                            <i className="fas fa-bookmark text-orange-500 text-xs"></i>
                                            {tipo.nombre}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => abrirEditar(tipo)}
                                                className="text-orange-600 hover:text-orange-800 p-1.5 rounded hover:bg-orange-50 transition-colors" title="Editar">
                    ...
                    <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 text-xs text-gray-400">
                        {tipos.length} tipo(s) de recomendación registrado(s)
                    </div>
                </div>
            )}
        </div>
