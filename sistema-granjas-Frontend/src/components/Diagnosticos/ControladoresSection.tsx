...
// ----------------------------------------------------------------------
// Componentes auxiliares (RealFotosSection, ImageModal)
// ----------------------------------------------------------------------
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white p-4 rounded-lg max-w-2xl max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        <button type="button" className="float-right text-gray-600 hover:text-gray-900 text-xl font-bold" onClick={onClose}>
          ×
        </button>
...
// ----------------------------------------------------------------------
// Mapa de imágenes de referencia
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Props y Ref del componente principal
// ----------------------------------------------------------------------
export interface ControladoresSectionRef {
  validate: () => boolean;
...
// ----------------------------------------------------------------------
export const ControladoresSection = forwardRef<ControladoresSectionRef, ControladoresSectionProps>(
  ({ plantas, caracterizacion, onCampoChange }, ref) => {
    ...
    // ------------------------------------------------------------------
    // Lógica de cambios (igual que antes, pero con limpieza de errores)
    // ------------------------------------------------------------------
    ...
    const getNoAplica = (codigo: string): boolean => {
      return caracterizacion[`controladores_${codigo}_noAplica`] === 'true';
    };

    const handleNoAplicaChange = (codigo: string, checked: boolean) => {
      const prefix = `controladores_${codigo}`;
      handleChange(`${prefix}_noAplica`, checked);
      if (checked) {
        [
          'insectos', 'insectos_otro', 'insectos_otro_fotos',
          'microbianos', 'microbianos_otro', 'microbianos_otro_fotos',
          'evidencias', 'evidencias_otro', 'evidencias_otro_fotos',
          'nivel'
        ].forEach(campo => handleChange(`${prefix}_${campo}`, ''));
      }
    };

    // Inicialización por defecto con "No se observaron"
    useEffect(() => {
      plantas.forEach((planta) => {
        const codigo = planta.codigo;
        const baseKey = `controladores_${codigo}`;
        const grupos = [
    ...
    const validate = (): boolean => {
      ...
      plantas.forEach((planta) => {
        ...
        // Validar grupo insectos
        const insectosKey = `${baseKey}_insectos`;
        const insectos = safeParseArray(caracterizacion[insectosKey]);
        if (insectos.length === 0) {
          nuevosErrores[`${insectosKey}_error`] = 'Debe seleccionar al menos una opción.';
          isValid = false;
        } else if (insectos.includes('Otro')) {
          const otroTexto = caracterizacion[`${baseKey}_insectos_otro`] || '';
          if (!otroTexto.trim()) {
            nuevosErrores[`${baseKey}_insectos_otro_error`] = 'Debe especificar el otro insecto benéfico.';
            isValid = false;
          }
          const fotosPrefix = `${baseKey}_insectos_otro_fotos`;
          const fotos = filesMap.get(fotosPrefix) || [];
          if (fotos.length === 0) {
            nuevosErrores[`${fotosPrefix}_error`] = 'Debe subir al menos una foto del insecto observado.';
            isValid = false;
          }
        }

        // Validar grupo microbianos
        const microbianosKey = `${baseKey}_microbianos`;
        const microbianos = safeParseArray(caracterizacion[microbianosKey]);
        if (microbianos.length === 0) {
          nuevosErrores[`${microbianosKey}_error`] = 'Debe seleccionar al menos una opción.';
          isValid = false;
        } else if (microbianos.includes('Otro')) {
          const otroTexto = caracterizacion[`${baseKey}_microbianos_otro`] || '';
          if (!otroTexto.trim()) {
            nuevosErrores[`${baseKey}_microbianos_otro_error`] = 'Debe especificar el otro controlador microbiano.';
            isValid = false;
          }
          const fotosPrefix = `${baseKey}_microbianos_otro_fotos`;
          const fotos = filesMap.get(fotosPrefix) || [];
          if (fotos.length === 0) {
            nuevosErrores[`${fotosPrefix}_error`] = 'Debe subir al menos una foto del microbiano observado.';
            isValid = false;
          }
        }

        // Validar grupo evidencias
        const evidenciasKey = `${baseKey}_evidencias`;
        const evidencias = safeParseArray(caracterizacion[evidenciasKey]);
        if (evidencias.length === 0) {
          nuevosErrores[`${evidenciasKey}_error`] = 'Debe seleccionar al menos una opción.';
          isValid = false;
        } else if (evidencias.includes('Otro')) {
          const otroTexto = caracterizacion[`${baseKey}_evidencias_otro`] || '';
          if (!otroTexto.trim()) {
            nuevosErrores[`${baseKey}_evidencias_otro_error`] = 'Debe especificar la otra evidencia.';
            isValid = false;
          }
          const fotosPrefix = `${baseKey}_evidencias_otro_fotos`;
          const fotos = filesMap.get(fotosPrefix) || [];
          if (fotos.length === 0) {
            nuevosErrores[`${fotosPrefix}_error`] = 'Debe subir al menos una foto de la evidencia.';
            isValid = false;
          }
        }

        // Validar nivel de presencia
        const nivel = caracterizacion[`${baseKey}_nivel`] || '';
        if (!nivel) {
          nuevosErrores[`${baseKey}_nivel_error`] = 'Debe seleccionar el nivel de presencia.';
          isValid = false;
        }
      });
    ...
    const insectosOpciones = [
      'Coccinélidos', 'Crisopas', 'Avispas parasitoides', 'Tamarixia radiata', 'Fidiobia sp.',
      'No se observaron', 'Otro'
    ];
    const microbianosOpciones = [
      'Beauveria', 'Lecanicillium', 'Metarhizium', 'Bacillus',
      'No se observaron', 'Otro'
    ];
    const evidenciasOpciones = [
      'Huevos de artrópodos benéficos', 'Larvas depredando', 'Plagas parasitadas',
      'Micelio en insectos', 'Insectos benéficos en estados inmaduros', 'Insectos benéficos adultos',
      'No se observaron evidencias', 'Otro'
    ];
              ...
              {!noAplica && (
                <div className="space-y-4">
                  ...
                  <div className="border p-3 rounded bg-gray-50">
                        ...
                        <div key={opcion} className="flex items-center gap-2">
                          <label className="flex items-center text-sm">
                            <input
                              ...
                              onChange={(e) =>
                                handleGrupoChange(baseKey, 'insectos', opcion, e.target.checked, opcionNingunoInsectos)
                              }
                              className="mr-2"
                            />
                            {opcion}
                    ...
                    {errores[`${baseKey}_insectos_error`] && (
                      <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_insectos_error`]}</p>
                    )}
                    {tieneOtroInsectos && (
                      <div className="mt-3">
                        ...
                        />
                        {errores[`${baseKey}_insectos_otro_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_insectos_otro_error`]}</p>
                        )}
                        <RealFotosSection
                        ...
                        />
                        {errores[`${baseKey}_insectos_otro_fotos_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_insectos_otro_fotos_error`]}</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border p-3 rounded bg-gray-50">
                        ...
                        <div key={opcion} className="flex items-center gap-2">
                          <label className="flex items-center text-sm">
                            <input
                              ...
                              onChange={(e) =>
                                handleGrupoChange(baseKey, 'microbianos', opcion, e.target.checked, opcionNingunoMicrobianos)
                              }
                              className="mr-2"
                            />
                            {opcion}
                    ...
                    {errores[`${baseKey}_microbianos_error`] && (
                      <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_microbianos_error`]}</p>
                    )}
                    {tieneOtroMicrobianos && (
                      <div className="mt-3">
                        ...
                        />
                        {errores[`${baseKey}_microbianos_otro_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_microbianos_otro_error`]}</p>
                        )}
                        <RealFotosSection
                          ...
                          onOpenImage={(url) => setModalImage(url)}
                        />
                        {errores[`${baseKey}_microbianos_otro_fotos_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_microbianos_otro_fotos_error`]}</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border p-3 rounded bg-gray-50">
                        ...
                        <div key={opcion} className="flex items-center gap-2">
                          <label className="flex items-center text-sm">
                            <input
                              ...
                              onChange={(e) =>
                                handleGrupoChange(baseKey, 'evidencias', opcion, e.target.checked, opcionNingunoEvidencias)
                              }
                              className="mr-2"
                            />
                            {opcion}
                    ...
                    {errores[`${baseKey}_evidencias_error`] && (
                      <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_evidencias_error`]}</p>
                    )}
                    {tieneOtroEvidencias && (
                      <div className="mt-3">
                        <label className="block text-sm text-gray-600 mb-1">Otro (especifique) *</label>
                        ...
                        />
                        {errores[`${baseKey}_evidencias_otro_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_evidencias_otro_error`]}</p>
                        )}
                        <RealFotosSection
                          ...
                          onOpenImage={(url) => setModalImage(url)}
                        />
                        {errores[`${baseKey}_evidencias_otro_fotos_error`] && (
                          <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_evidencias_otro_fotos_error`]}</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border p-3 rounded bg-gray-50">
                    ...
                    {errores[`${baseKey}_nivel_error`] && (
                      <p className="text-red-600 text-xs mt-1">{errores[`${baseKey}_nivel_error`]}</p>
                    )}
                  </div>
                </div>
              )}
